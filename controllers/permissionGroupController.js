...
exports.update = async function(req, res, next) {
  try {
    ...
    if(await ModelsSharedFunctions.exists(DatasetModel.collection(), uuid)) {
      ...
      let template = await ModelsSharedFunctions.latestPublished(TemplateModel.collection(), template_uuid);
      for(let a_user of users) {
        if(!(await ModelsSharedFunctions.userHasAccessToPublishedResource(template, a_user, PermissionGroupModel))) {
