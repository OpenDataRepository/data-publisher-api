...
router.get('/:uuid/draft', ensureLoggedIn(), validateUuid, templateController.draft_get);
router.get('/:uuid/draft_existing', getUserFromToken, validateUuid, templateController.draft_existing);
router.get('/:uuid/latest_persisted', getUserFromToken, validateUuid, templateController.get_latest_persisted);
router.get('/:uuid/last_update', ensureLoggedIn(), validateUuid, templateController.get_last_update);
router.get('/:uuid/:timestamp', getUserFromToken, validateUuid, validateTimestamp, templateController.get_persisted_before_timestamp);
