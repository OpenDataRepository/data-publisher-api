...
async function validateAndCreateOrUpdate(session, input_field, user, updated_at) {
  ...
  if (input_field.uuid) {
    ...
    if (!(await UserPermissionsModel.has_permission(user, input_field.uuid, PermissionGroupModel.PermissionTypes.edit))) {
...
async function persistField(session, uuid, last_update, user) {
  ...
  let has_permission = await UserPermissionsModel.has_permission(user, uuid, PermissionGroupModel.PermissionTypes.edit, session);
  if(!has_permission) {
    throw new Util.PermissionDeniedError();
  }
...
exports.importField = async function(field, user, updated_at, session) {
  ...
  if(uuid) {
    if(!UserPermissionsModel.has_permission(user, uuid, PermissionGroupModel.PermissionTypes.edit, session)) {
